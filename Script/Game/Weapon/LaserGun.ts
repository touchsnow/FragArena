...
@ccclass('LaserGun')
export class LaserGun extends ContinuousGun {
    ...
    shot() {
        if (!this.bullet) {
            let node = instantiate(resources.get("Bullet/LaserBullet")) as Node
            node.setParent(find("BulletPoint"))
            let bullet = node.getComponent(ContinuousBullet)
            if (bullet) {
                this.bullet = bullet
                let args = [this.firePoint, this.rayPoint, this.player, 0, 0]
    ...
    backShot() {
        if (!this.backBullet) {
            ...
            if (bulletNode) {
                ...
                let args = [this.firePoint, this.rayPoint, this.player, 180, -1]
    ...
    RigthFrontShot() {
        if (!this.rightFrontBullet) {
            ...
            if (bulletNode) {
                ...
                let args = [this.firePoint, this.rayPoint, this.player, 45, 0]
